...
#if 0
COMMAND_HANDLER(STM32H5_FlashLockCommand) {
	int ret;
	if (CMD_ARGC == 1) {
		struct flash_bank* bank;
		if ((ret = CALL_COMMAND_HANDLER(flash_command_get_bank, 0, &bank)) == ERROR_OK) {
...
static const struct command_registration STM32H5_ExecCommandHandlers[] = {
	{
		.name = "mass_erase",
		.handler = STM32H5_MassEraseCommand,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
...
static const struct command_registration STM32H5_CommandHandlers[] = {
	{
		.name = "stm32h5xx",
		.mode = COMMAND_ANY,
		.help = "stm32h5xx flash command group",
		.usage = "",
		.chain = STM32H5_ExecCommandHandlers,
	},
	COMMAND_REGISTRATION_DONE
};
...
const struct flash_driver stm32h5x_flash = {
	.name					= "stm32h5xx",
	.commands				= STM32H5_CommandHandlers,
	.flash_bank_command		= NULL, //stm32l4_flash_bank_command,
	.erase					= NULL, //stm32l4_erase,
	.protect				= NULL, //stm32l4_protect,
	.write					= NULL, //stm32l4_write,
	.read					= NULL, //default_flash_read,
	.probe					= NULL, //stm32l4_probe,
	.auto_probe				= NULL, //stm32l4_auto_probe,
